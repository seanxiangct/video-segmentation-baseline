...
from keras.layers import Input, Dense, TimeDistributed, merge, Lambda, multiply, add, Bidirectional, Concatenate
...
def BidirLSTM(n_nodes, n_classes, n_feat, max_len=None,
    ...
    #     model = Concatenate(axis=1)([model, model_backwards])
    #
    # model = TimeDistributed(Dense(n_classes, activation="softmax"))(model)
    #
    # model = Model(input=inputs, output=model)
